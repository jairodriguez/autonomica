...
const StatusIcon = ({ status }: { status: Agent['status'] }) => {
  ...
  switch (status) {
    case 'busy':
      return (
        <div className={`${baseClasses} relative`}>
          <ClockIcon className="w-3 h-3 text-blue-500 animate-spin" />
        </div>
      );
    case 'idle':
    ...
    case 'offline':
      return <div className={`${baseClasses} bg-gray-400 rounded-full`} />;
    default:
      return <div className={`${baseClasses} bg-gray-400 rounded-full`} />;
  }
};
...
export default function ProjectSidebar({
  ...
  return (
    <div className={`w-72 bg-white border-r border-gray-200 overflow-y-auto ${className}`}>
      {/* Header */}
      <div className="p-4 border-b border-gray-200">
        <h2 className="text-lg font-semibold text-gray-900">Projects</h2>
        <p className="text-sm text-gray-500">Manage your AI agent workforce</p>
      </div>
